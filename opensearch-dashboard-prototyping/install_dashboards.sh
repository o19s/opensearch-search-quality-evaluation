...
curl -s -X PUT $opensearch/srw_metrics_sample_data/ -H "Content-Type: application/json" -d @"$SCRIPT_DIR/srw_metrics_mappings.json"

     
echo "${MAJOR}Populating index srw_metrics_sample_data with sample metric data${RESET}"
curl -s -H 'Content-Type: application/x-ndjson' -XPOST "$opensearch/srw_metrics_sample_data/_bulk?pretty=false&filter_path=-items" --data-binary @"$SCRIPT_DIR/sample_data.ndjson" 

echo "${MAJOR}\nInstalling Quality Evaluation Framework Dashboards${RESET}"
curl -X POST "$opensearch_dashboard/api/saved_objects/_import?overwrite=true" -H "osd-xsrf: true" --form file=@"$SCRIPT_DIR/search_dashboard.ndjson" > /dev/null
